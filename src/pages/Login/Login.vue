            ...
            <div :class="{on: loginWay}">
              <section class="login_message">
                ...
                <button :disabled="!rightPhone"
                        ...
                        :class="{right_phone: rightPhone}" @click.prevent="getCode">{{computeTime > 0 ? `已发送(${computeTime})`:'获取验证码'}}</button>
              </section>
    ...
    async login () {
      ...
      if (this.loginWay) { // 短信登录
        ...
        if (!this.name) {
         ...
         // 发送ajax请求短信登录
        result = await reqPwdLogin(this.phone, this.code, this.captcha)
      }
      // 停止计时
          if (this.computeTime) {
            this.computeTime = 0
